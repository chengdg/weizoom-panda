...
class NewProduct(resource.Resource):
    ...
    def get(request):
        ...
        if product_id:
            ...
            product_data = {
                ...
                'valid_time_from': '' if not product.valid_time_from else product.valid_time_from.strftime(
                    "%Y-%m-%d %H:%M"),
            ...
            # 获取商品图片
            product_image_ids = [product_image.image_id for product_image in
                                 models.ProductImage.objects.filter(product_id=product_id)]
    ...
    def api_delete(request):
        ...
        change_rows = models.Product.objects.filter(id=request.POST['id']).update(is_deleted=True)
        if change_rows > 0:
