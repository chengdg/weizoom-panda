...
Scenario:1  新增商品后,运营账号同步商品
	...
	Then yunying查看商品列表
		...
		|product_name|consumer|	   classify      | source | sale |    status    |
		|  武汉鸭脖  |爱伲咖啡|   生活用品-零食  |        | 0.00 |已入库，已同步|
		| 耐克男鞋   |爱伲咖啡|   电子数码-耳机  |        | 0.00 |已入库，已同步|
	...
	When aini查看商品列表
		...
		|武汉鸭脖 |  爱伲咖啡   |  生活用品-零食   |    8.00  | 0.00  |2016-07-25 16:30| 已上架 |
		|耐克男鞋 |  爱伲咖啡   |  电子数码-耳机   |   29.00  | 0.00  |2016-07-25 16:30| 已上架 |
...
Scenario:2 修改商品后,再选择一个自营平台同步
	...
	When yunying同步商品'周黑鸭'
		"""
		{
		"synchronous_goods":"微众妈妈"
		}
		"""
	When yunying同步商品'耐克男鞋'
		"""
		{
		"synchronous_goods":"微众学生"
		}
		"""
	Then yunying查看商品列表
		|id|  name  |   classify  |supplier|source|set_price|sale_price| sales |   creat_time   | status |operate|
